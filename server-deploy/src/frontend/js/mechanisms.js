...
const Mechanisms = {
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                ...
                if (elapsed >= requiredTime) {
                    // 记录查看消息事件
                    if (typeof Tracking !== 'undefined' && Tracking.trackViewMessage) {
                        Tracking.trackViewMessage(elapsed);
                    }
    ...
    setupHighlightTask() {
        ...
        if (typeof Tracking !== 'undefined' && Tracking.trackHighlightTask) {
            Tracking.trackHighlightTask('show');
        }
    ...
    handleTextSelection(selectedText, messageElement) {
        ...
        // 记录高亮任务选择关键词事件
        if (typeof Tracking !== 'undefined' && Tracking.trackHighlightTask) {
            Tracking.trackHighlightTask('select', selectedText);
        }
        ...
        rewardCard.innerHTML = `
            <div class="text-center">
                ...
                <p class="text-gray-600 mb-6">
                    恭喜！你已完成6轮深度探索，已掌握 ${AppState.topicConfig?.topic_name || 'C++ 指针'}概念！
                </p>
    ...
    async lightUpConcept(conceptName) {
        ...
        // 保存到数据库
        try {
            if (typeof API !== 'undefined' && API.saveConcept) {
