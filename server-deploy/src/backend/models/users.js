...
const Users = {
    ...
    async create(userData) {
        const result = await db.execute(
            `INSERT INTO users (student_id, group_id, condition_depth, condition_turns, consent_agreed, max_turns, target_turns, password)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                userData.student_id,
                userData.group_id,
                ...
                userData.password || '123' // 默认密码为'123'，如果提供了密码则使用提供的密码
            ]
        );
